...
//                         scrollEnabled={false}      // ✅ disables FlatList scroll to avoid nested conflict
//                         nestedScrollEnabled={true} // ✅ ensures smooth behavior if inside ScrollView
//                     />
//                 )}
...
export default function AdminDashboard() {
    ...
    // ✅ Login check
    const handleLogin = () => {
        if (adminName.trim() && shopName.trim()) {
            setIsLoggedIn(true);
            Alert.alert("✅ Login Success", `Welcome ${adminName}`);
        } else {
            Alert.alert("Error", "Enter both admin name and shop name.");
        }
    };

    // ✅ Register device
    const handleRegister = async () => {
        if (!isLoggedIn) return Alert.alert("Error", "Login required first.");
        if (!imei || !owner) return Alert.alert("Error", "Enter both fields.");
        ...
        try {
            ...
            if (res.ok) {
                ...
                setImei("");
                setOwner("");
            } else Alert.alert("Error", data.msg || "Registration failed.");
        } catch {
            Alert.alert("Error", "Server not responding.");
        }
    };
